package org.cosmo.remotetransport.inbuilt;
/**
 * @author Sonu Aryan
 */
public abstract class InbuiltServices {
    public static final String CONSOLE_SERVICES="c_s";
    public static final String FILE_SYSTEM_SERVICES="f_s_s";
    public static final String OS_SERVICES="os_s";
    public static final String DEVICE_INFO_PROVIDER_SERVICES="d_i_p_s";
    public static final String SERVICE_INFO_PROVIDER_SERVICES="s_i_p_s";
}
